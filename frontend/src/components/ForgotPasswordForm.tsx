...
export const ForgotPasswordForm: React.FC<ForgotPasswordFormProps> = ({ onBackToLogin }) => {
  ...
  const handleRequestOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://sankalp-deploy-1.onrender.com/api/forgot-password', {
        method: 'POST',
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://sankalp-deploy-1.onrender.com/api/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://sankalp-deploy-1.onrender.com/api/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="w-full max-w-md relative z-10">
      ...
      <div className="form-container bg-black/80 backdrop-blur-lg border border-gray-800 p-8 rounded-2xl shadow-xl">
        <div className="flex items-center justify-between mb-8 opacity-0" style={{ animation: 'slideInUp 0.5s ease-out 0.1s forwards' }}>
          <button 
          ...
          <div className="flex items-center">
            <div className="bg-primary-900/30 p-3 rounded-full">
              {step === 'email' && <Mail className="w-5 h-5 text-primary-300" />}
              {step === 'otp' && <KeyRound className="w-5 h-5 text-primary-300" />}
              {(step === 'reset' || step === 'success') && <Lock className="w-5 h-5 text-primary-300" />}
            </div>
